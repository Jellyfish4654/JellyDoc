...
<Tabs items={['BaseOpMode Class', 'Hanger Class', 'TeleOp Example']}>
  <Tabs.Tab>
    ```java
    package org.firstinspires.ftc.teamcode.Framework;
    ...
    public abstract class BaseOpMode extends LinearOpMode {
        ...
        protected void initHardware() {
            //Hanger
            DcMotor hangerMotor = hardwareMap.get(DcMotor.class, "hangerMotor");
            hanger = new Hanger(hangerMotor);
        }
    }
    ```
  ...
  <Tabs.Tab>
    ...
    public class Hanger {
        ...
        public void update() {
            ...
            double command = calculate(targetPosition, currentPosition);
            hangerMotor.setPower(command);
        }

        private double calculate(double reference, double position) {
            if (Math.abs(reference - position) < TOLERANCE) {
                return 0.0;
